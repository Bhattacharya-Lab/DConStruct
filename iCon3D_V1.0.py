...
class iCon3D():
                        ...
                        for j in range(i+1, n):
                                if self.CM[i][j] == 1:
                                        ...
                                        if (abs(i - j) < 6):
                                                ...
                                                D[i][j] = self.gdfCM[i][j] * 5.72 #gdfuz3D
        ...
        def embed3(self):
                ...
                D = self.guess_dist()
                D = self.ChunksImproveDist(D)
                mds = MDS(n_components=3, max_iter=300000, eps=1e-9, random_state = seed,
                ...
                for i in range(20):
                        ...
                        D = self.guess_dist()
                        D = self.ChunksImproveDist(D)
                        seed1 = np.random.RandomState(seed = int(i+int(no)))
        ...
        def toPDB(self, pos, suffix):
                f = open(output + '/' + aa_file + suffix, 'w') #write pdb in this file
        ...
        def correctCM(self, pos):
                for i in range(len(self.CM)):
                        ...
                        for j in range(len(self.CM)):
                                ...
                                if (self.CM[i][j] == 1 and dij > self.t):
                                        #r = dij - self.t
        ...
        def perturb(self, pos, eps):
                for i in range(len(self.CM)):
                        ...
                        for j in range(len(self.CM)):
                                dij = math.sqrt((pos[i][0] - pos[j][0]) * (pos[i][0] - pos[j][0]) + (pos[i][1] - pos[j][1]) * (pos[i][1] - pos[j][1]) + (pos[i][2] - pos[j][2]) * (pos[i][2] - pos[j][2]))
                                if (self.t - eps < dij <= self.t and self.CM[i][j] == 1):
...
##Credit: Dr. Debswapna Bhattacharya##
        ...
        def generateModellerScript(self):
                ...
                f.write('                               if (self.CM[i][j] == 1 and d > '+str(self.t)+'): \n')
                f.write('                                       firstres = ct + \':\' + str(i+1) \n')
                f.write('                                       secondres = ct + \':\' + str(j+1) \n')
                f.write('                                       if(rrlines[0][i] == \'G\'): \n')
                f.write('                                               firstres = \'CA:\' + str(i+1) \n')

                f.write('                                       if(rrlines[0][j] == \'G\'): \n')
